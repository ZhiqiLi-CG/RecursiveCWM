...
const timber=mat('#704019'),edge=mat('#8e4b14'),plaster=mat('#d4c5a5'),stone=[mat('#737e78'),mat('#969e90'),mat('#647578'),mat('#b0ac91')],sail=mat('#576362'),rail=mat('#494d42'),roof=mat('#a86437');
...
function mill(ctx,foot,scale,angle){let root=new THREE.Group();root.name=scale<1?'western-windmill':'eastern-windmill';root.position.copy(ctx.pixelToWorld(...foot,.06));root.scale.setScalar(scale);root.rotation.y=-angle;
 ...
 for(let i=0;i<8;i++){let a=Math.PI/8+i*Math.PI/4, b=a+Math.PI/4;let v=(r,y,t)=>[r*Math.sin(t),y,r*Math.cos(t)];beam(root,v(.353,.31,a),v(.181,1.45,a),.041,.039,timber);for(let [y0,y1] of [[.34,.65],[.69,1.08],[1.09,1.41]]){let r0=.35-(y0-.3)*.149,r1=.35-(y1-.3)*.149;beam(root,v(r0,y0,a),v(r1,y1,b),.032,.030,timber);}}
 for(let [y,r,h] of [[.32,.365,.085],[.64,.33,.085],[1.09,.245,.051],[1.43,.207,.061]])cone(root,r,r,y,h,edge);
 ...
 let hubY=1.30,forward=.47;beam(root,[0,hubY,-.10],[0,hubY,forward+.06],.082,.082,timber);
 let rotor=new THREE.Group();rotor.position.set(0,hubY,forward);rotor.rotation.z=-Math.PI/4;root.add(rotor);
 for(let k=0;k<4;k++){let wing=new THREE.Group();wing.rotation.z=k*Math.PI/2;rotor.add(wing);beam(wing,[0,0,0],[0,.80,0],.044,.035,timber);
  ...
  for(let j=0;j<9;j++)box(wing,.028,.29+j*.061,.02,.174,.040,.022,sail);
 }
 ...
 return root;}
export function build(ctx){let g=new THREE.Group();g.name='east-windmills';g.add(mill(ctx,[1169,389],.73,.28),mill(ctx,[1236,374],1,.28));return g;}
