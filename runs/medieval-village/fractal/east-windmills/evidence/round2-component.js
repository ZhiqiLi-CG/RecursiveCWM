...
const timber=mat('#704019'),edge=mat('#6e390e'),plaster=mat('#dcc7a1'),stone=[mat('#737e78'),mat('#969e90'),mat('#647578'),mat('#b0ac91')],sail=mat('#576362'),rail=mat('#494d42'),roof=mat('#885437');
...
function mill(ctx,foot,scale,angle){let root=new THREE.Group();root.name=scale<1?'western-windmill':'eastern-windmill';root.position.copy(ctx.pixelToWorld(...foot,.06));root.scale.setScalar(scale);root.rotation.y=-angle;
 ...
 let rotor=new THREE.Group();rotor.position.set(0,hubY,forward);rotor.rotation.z=-Math.PI/4;rotor.scale.setScalar(scale<1?1:.9);root.add(rotor);
 ...
 return root;}
export function build(ctx){let g=new THREE.Group();g.name='east-windmills';g.add(mill(ctx,[1169,387],.73,.28),mill(ctx,[1233.5,372],1,.28));return g;}
