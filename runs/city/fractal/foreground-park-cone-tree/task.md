# solve(foreground-park-cone-tree) — 统一递归求解器 @深度2

**这是已批准的执行工单。**可逆选择按你的推荐直接干,不停门。

链目录:runs/pilot/city-full-recursive-r1;你的节点目录:runs/pilot/city-full-recursive-r1/fractal/foreground-park-cone-tree/
...
3. **再整体**(被叫醒后):孩子的 part.json 已就位;把它们接进你的层,回本层
   ...
   否则进入收官。
4. **收官**:写 runs/pilot/city-full-recursive-r1/fractal/foreground-park-cone-tree/part.json(本层最终组件,含
