...
const SCALE=Number(process.env.SCALE||6);const args=process.argv.slice(2),opt=k=>args.includes(k)?args[args.indexOf(k)+1]:undefined;
...
const server=http.createServer((req,res)=>{
 if(req.url==='/three.js'){res.setHeader('Content-Type','application/javascript');return res.end(fs.readFileSync('/data/zhiqi/CodeWorld2/.render-tools/node_modules/three/build/three.module.js','utf8')+"\n;(function(){try{Object.defineProperty(WebGLRenderer.prototype,'render',{configurable:true,get(){return undefined},set(fn){const self=this;Object.defineProperty(self,'render',{configurable:true,writable:true,value:function(s,c){window.__lastRenderer=self;window.__lastScene=s;window.__lastCamera=c;return fn.call(self,s,c)}})}});}catch(e){}})();");}
...
try{const page=await browser.newPage({viewport:{width:w,height:h},deviceScaleFactor:1});const errors=[];page.on('pageerror',e=>errors.push(String(e)));page.on('console',m=>{if(m.type()==='error')errors.push(m.text());});
 await page.goto(`http://127.0.0.1:${server.address().port}`);await page.waitForFunction(()=>window.RENDER_DONE===true,null,{timeout:60000});await page.evaluate((k)=>{const r=window.__lastRenderer;r.setPixelRatio(k);r.setSize(r.domElement.clientWidth||r.domElement.width,r.domElement.clientHeight||r.domElement.height,false);r.render(window.__lastScene,window.__lastCamera);},SCALE);const data=await page.evaluate(()=>window.__lastRenderer.domElement.toDataURL('image/png'));fs.writeFileSync(validate(out),Buffer.from(data.split(',')[1],'base64'));
