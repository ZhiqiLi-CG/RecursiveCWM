...
const scene=opt('--scene')||'../part.json',camera=opt('--camera')||'../../camera-contract.json',out=opt('--out')||'render0.png',view=opt('--view');
...
const sourceSHA=createHash('sha256').update(fs.readFileSync(validate(scene))).digest('hex'); if(sourceSHA!==payload.camera.source_part_sha256)throw Error('Frozen scene digest mismatch');
