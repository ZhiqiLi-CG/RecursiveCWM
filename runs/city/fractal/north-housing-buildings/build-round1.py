...
for n,(u,v) in enumerate([(78,153.5),(129,128),(180,102.5),(231,77)],1):
 ...
 for z in [0,-.80]:face(q+f'gable{z}',[(-.6,.755,z),(0,.755,z),(-.30,1.065,z)],'#e2d48c')
 def slope(x):return 1.065-(x+.30)*1.033333
 # Broad roof plane extends over the front door and porch only on its far half.
 roofpatch(q+'roof-main',[(-.30,1.065,.055),(-.30,1.065,-.88),(.20,slope(.20),-.88),(.20,slope(.20),-.45),(.035,slope(.035),-.45),(.035,slope(.035),.055)],'#ce50e7')
 ...
 box(q+'porch',.15,.080,-.665,.33,.06,.36,['#d062e4','#b63dd2','#c353df'])
 for k,z in enumerate([-.495,-.835]):box(q+f'porch-post{k}',.285,.348,z,.030,.49,.026,['#edc78f','#dbad6d','#e9bf85'])
 box(q+'step',.35,.040,-.665,.12,.035,.29,['#c887db','#b26fc5','#c38bd6'])
 # Two dormer bodies rise through the roof slope and have small purple pitched tops.
 for k,z in enumerate([-.18,-.59]):
  box(q+f'dormer{k}-body',-.115,.961,z,.22,.22,.22,['#ebd88e','#eee2a6','#dcca85'])
 ...
 box(q+'chimney',-.44,1.10,-.035,.095,.25,.105,['#c9a6cb','#936894','#a57dae'])
 box(q+'chimney-cap',-.44,1.23,-.035,.125,.035,.135,['#c6a0d0','#aa7bb6','#b387bc'])
 box(q+'chimney-hole',-.44,1.250,-.035,.056,.007,.059,'#826686')
